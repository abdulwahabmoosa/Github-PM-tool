...
export default function Dashboard({ user, onLogout }) {
  ...
  // If selected repo was disconnected, fall back to first remaining
  useEffect(() => {
  ...
  async function handleConnect(payload) {
    try {
      ...
      showToast(`Failed to connect repo: ${err.message}`, 'error');
    }
  }
  ...
  async function handleDisconnect(repoId) {
    try {
      ...
      if (repoId === selectedRepoId) {
        setSelectedRepoId(connectedRepos.filter((r) => r.id !== repoId)[0]?.id ?? null);
      }
      refreshRepos();
    } catch (err) {
      showToast(`Failed to disconnect repo: ${err.message}`, 'error');
    }
  }
