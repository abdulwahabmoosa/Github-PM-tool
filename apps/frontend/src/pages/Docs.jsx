        ...
        <main className="flex-1 py-10 md:pl-8 md:border-l border-slate-200 dark:border-slate-800 min-w-0">
          ...
          <Section>
            ...
            <ol className="list-decimal list-inside text-base text-slate-700 dark:text-slate-300 mb-4 space-y-2 leading-relaxed">
              <li>You push an annotated git tag with a special name like <C>task-3-claim</C></li>
          ...
          <Section>
            ...
            <P>
              Push an annotated git tag named <C>task-N-{'{verb}'}</C> to drive a transition.
            </P>
            <Table
          ...
          <Section>
            ...
            <P>
              This is why TaskMaster requires annotated tags — the tagger metadata is
              independent of the commit being tagged, ensuring TaskMaster attributes the
              workflow action to the person who ran <C>git tag -a</C>, not to whoever
              happened to author the underlying commit.
            </P>
          </Section>
          ...
          <Section>
            ...
            <Callout>
              <strong>Annotated tags required.</strong>{' '}
              TaskMaster requires annotated tags (created with the <C>-a</C> flag).
              Annotated tags carry tagger identity — name, email, timestamp — set by the
              developer who runs <C>git tag -a</C>, regardless of which commit they point
              at. Lightweight tags (created with just <C>git tag &lt;name&gt;</C>) inherit
              metadata from the underlying commit's author, which would misattribute workflow
              actions. Lightweight tags are rejected at ingestion.
            </Callout>
          ...
          <Section>
            ...
            <P>
              Commits are attributed first by message reference, then by branch match.
              A commit with no task reference still appears in the weekly commit count
              but is not attributed to any specific task.
            </P>
            <H3>Attributing commits to tasks</H3>
            <P>
              To attribute a commit to a specific task, reference the task number in the
              commit message using any of these formats:
            </P>
            <Pre>{`#5 fixed bug\n(#5) refactored login\ntask-5 added new feature\ntask#5 made changes\ntask 5 work in progress\ntask5 done`}</Pre>
            <P>
              The commit will appear in that task's activity counter on the dashboard.
              Commits without a task reference still appear in the weekly commit count
              but are not attributed to any specific task.
            </P>
          </Section>
          ...
          <Section>
            ...
            <FaqItem q="Why didn't my tag work?">
              ...
              active, pushing as a user who isn't the task's assignee (for help/review/done
              verbs), or using a lightweight tag (no <C>-a</C> flag) — TaskMaster only
              accepts annotated tags. To re-push correctly:{' '}
              <C>{"git tag -a <name> -m '<message>' && git push --tags"}</C>.
              Check the task's history timeline — skipped events appear there with a reason.
            </FaqItem>

            <FaqItem q="What if my commit message references a different task number than my tag?">
              Commit-message attribution takes priority. If the message contains a task
              reference (e.g. <C>#5</C> or <C>task-5</C>), the commit is attributed to
              that task — not necessarily the task the tag targets. Use an explicit
              reference in the commit message to ensure the commit lands on the right task.
            </FaqItem>
