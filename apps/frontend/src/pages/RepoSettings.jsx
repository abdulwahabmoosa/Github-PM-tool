...
export default function RepoSettings({ user, onLogout }) {
  ...
  async function handleModeChange(newMode) {
    try {
      ...
      showToast(err.status === 403 ? "Only the repo owner can change mode." : `Failed: ${err.message}`, 'error');
    }
  }
  ...
  async function handleRoleChange(userId, newRole) {
    try {
      ...
      const msg = err.status === 400 ? 'Cannot demote the last admin. Promote someone else first.' : `Failed: ${err.message}`;
      showToast(msg, 'error');
    }
  }
