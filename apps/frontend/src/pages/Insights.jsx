...
export default function Insights({ user, onLogout }) {
  ...
  const selectedRepoId = repos.find((r) => r.id === requestedRepoId)?.id
    ?? repos[0]?.id
    ?? null;
  ...
  useEffect(() => {
    ...
    if (!repos.find((r) => r.id === requestedRepoId)) {
      setSearchParams(
        (prev) => {
  ...
  }, [repos, reposLoading, requestedRepoId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="max-w-5xl mx-auto px-5 py-6">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          onClick={() => navigate(`/dashboard${selectedRepoId ? `?repo=${selectedRepoId}` : ''}`)}
      ...
      <RepoSelector
        ...
        onManage={() => navigate(`/dashboard${selectedRepoId ? `?repo=${selectedRepoId}` : ''}`)}
      />
